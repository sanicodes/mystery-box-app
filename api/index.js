...
app.get("/", (req, res) => {
    ...
    if (req.session.username && req.session.result) {
        const usernameExists = data.some(
            (entry) => entry.username === req.session.username,
        );

        if (usernameExists) {
            // If username exists, display the stored username and result
            return res.render("index", {
                message: `Hi ${req.session.username}, your box contains a ${req.session.result}`,
            });
        } else {
...
app.post("/open-box", (req, res) => {
    ...
    if (usernameExists) {
        ...
        const outcome =
            Math.random() < 0.5 ? "Bomb!" : "Move up the bracket item!";
        ...
        res.render("index", {
            message: `Hi ${username}! your box contains a ${outcome}`,
        });
    }
});
